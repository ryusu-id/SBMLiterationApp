...
`PublishDomainEventsAsync()` is replaced by `StoreOutboxMessagesAsync()`, which calls `IOutboxService.StoreManyAsync()`. Events are no longer dispatched synchronously — they are persisted as outbox messages and processed asynchronously.
...
| MODIFIED | `Backend/Data/UnitOfWork.cs` |
